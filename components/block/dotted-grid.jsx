...
/**
 * @param {{ className?: string, style?: import("react").CSSProperties, paused?: boolean, transparent?: boolean, dotHue?: number }} props
 */
export function DottedGrid({ className, style, paused = false, transparent = false, dotHue = 210 } = {}) {
  ...
  useEffect(() => {
    ...
    const drawDot = (x, y, radius, brightness, grayDisperseStrength, trailStrength, mouseStrength) => {
      ...
      ctx.beginPath();
      if (transparent) {
        ctx.fillStyle = `hsla(${dotHue}, ${saturation}%, 20%, ${alpha * 0.35})`;
      } else {
        ctx.fillStyle = `hsla(${dotHue}, ${saturation}%, ${finalLightness}%, ${alpha})`;
      }
  ...
  return (
    <button
      ...
      className={cn("relative block h-[28rem] w-full cursor-pointer overflow-hidden p-0 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring", !transparent && "bg-black", className)}
      style={style}
    >
      <canvas
        ref={canvasRef}
        aria-hidden="true"
        className={cn("block h-full w-full", !transparent && "bg-black")}
      />
      {!transparent && <span className="pointer-events-none absolute inset-0 bg-linear-to-b from-white/4 via-transparent to-black/40" />}
      {!transparent && <span className="pointer-events-none absolute inset-0 shadow-[inset_0_0_140px_rgba(0,0,0,0.95)]" />}
    </button>
  );
}
