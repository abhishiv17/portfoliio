    ...
    const initializeArrows = (canvas) => {
      ...
      if (text) {
        ...
        if (octx) {
          ...
          const fontSize = Math.floor(Math.min((canvas.width / text.length) * 1.5, 200));
          octx.font = `900 ${fontSize}px sans-serif`;
          octx.fillText(text, canvas.width / 2, canvas.height / 2);
          maskData = octx.getImageData(0, 0, canvas.width, canvas.height).data;
        }
      }

      for (let y = 0;y <= rows;y++) {
        for (let x = 0;x <= cols;x++) {
          const px = x * spacing + xPadding;
          const py = y * spacing + yPadding;
          
          if (maskData) {
            const idx = (Math.floor(py) * canvas.width + Math.floor(px)) * 4;
            if (idx >= 0 && idx < maskData.length && maskData[idx + 3] > 128) {
              arrows.push(new Arrow(new Point(px, py)));
            }
          } else {
            arrows.push(new Arrow(new Point(px, py)));
          }
      ...
      // Fallback: If mask failed or yielded nothing, fill the grid so it isn't invisible
      if (arrows.length === 0) {
        for (let y = 0;y <= rows;y++) {
          for (let x = 0;x <= cols;x++) {
